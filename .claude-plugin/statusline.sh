#!/usr/bin/env bash
# Status line for jkr. Reads `jkr gain --plain` and emits one compact line:
#   jkr · saved 8,432 / 28,673 (29%) · 36 cmds today
#
# If jkr isn't on PATH, prints nothing (a status line should fail silently;
...
set -e
command -v jkr >/dev/null 2>&1 || exit 0

OUT=$(jkr gain --plain 2>/dev/null) || exit 0

# Pull the three numbers + reduction. The format from `jkr gain --plain`:
#   tokens in             28,673
...
printf 'jkr · saved %s / %s (%s)' "$(fmt "$save_n")" "$(fmt "$in_n")" "$red"
