//! Read-only JobBoard MCP tool. Wraps `jkr jobs list`.
//!
//! We shell out to the `jkr` binary instead of pulling alloy/EVM-RPC into
//! jkr-mcp. The tradeoff: jkr-mcp must find a jkr binary on PATH (or via
//! JKR_BIN), but the dependency surface stays tiny.

use anyhow::{anyhow, Context, Result};
use std::process::Command;

/// Resolve the `jkr` binary. Honors `JKR_BIN` env override so tests and
/// non-PATH installs can point at a specific build.
fn jkr_bin() -> String {
    std::env::var("JKR_BIN").unwrap_or_else(|_| "jkr".to_string())
}

/// Run `jkr jobs list` with optional overrides and return its stdout.
///
/// `board`/`rpc_url` are passed through verbatim when set; otherwise the
/// `jkr` CLI uses its public-devnet defaults (DEFAULT_JOB_BOARD /
...
pub fn list(board: Option<&str>, rpc_url: Option<&str>, limit: Option<usize>) -> Result<String> {
    ...
    let out = cmd
        .output()
        .with_context(|| format!("spawn {bin}; ensure `jkr` is on PATH or set JKR_BIN"))?;
...
mod tests {
    ...
    /// parallel by default and JKR_BIN is process-global — separate
    ...
    fn list_argv_and_error_handling() {
        ...
        std::env::set_var("JKR_BIN", "/bin/echo");
