//! Optional JSONL trail when `JKR_NATIVE_SESSION_LOG=1` — lightweight “replay” of
...
pub fn maybe_append(cmd: &str, args: &[String], pipeline: &PipelineResult, exit_code: i32) {
    if std::env::var("JKR_NATIVE_SESSION_LOG").ok().as_deref() != Some("1") {
        return;
    }

    let home = dirs::home_dir().unwrap_or_default();
    let path = home.join(".jkr/native-handlers.jsonl");
