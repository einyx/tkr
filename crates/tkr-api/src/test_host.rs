...
impl TestHost {
    pub fn new(plugin: impl Into<String>) -> Self {
        Self {
            ...
            kv_pub: TestKv::default(), kv_priv: TestKv::default(), kv_sec: TestKv::default(),
            sqlite_pub: TestSqlite::default(), sqlite_priv: TestSqlite::default(), sqlite_sec: TestSqlite::default(),
...
impl Host for TestHost {
    fn plugin_name(&self) -> &str { &self.plugin }
    fn bus(&self) -> &dyn Bus { &self.bus }
    fn vault(&self) -> &dyn Vault { &self.vault }
...
impl TestBus {
    ...
    {
        self.handlers.lock().unwrap().insert((target.into(), method.into()), Box::new(f));
    }
...
impl Bus for TestBus {
    fn request(&self, req: Request) -> Result<Reply> {
        ...
        match handlers.get(&key) {
            Some(h) => h(req),
            None => Err(Error::UnknownMethod(format!("{}::{}", req.target, req.method))),
        }
    }
...
impl Vault for TestVault {
    fn state(&self) -> SealState { *self.state.lock().unwrap() }
...
#[derive(Default)]
pub struct TestKv { store: Mutex<HashMap<String, Value>> }
...
#[derive(Default)]
pub struct TestSqlite { calls: Mutex<Vec<(String, Vec<Value>)>> }

impl Sqlite for TestSqlite {
    fn execute(&self, sql: &str, params: &[Value]) -> Result<u64> {
        self.calls.lock().unwrap().push((sql.into(), params.to_vec()));
        Ok(0)
    }
...
impl TestSqlite {
    pub fn calls(&self) -> Vec<(String, Vec<Value>)> { self.calls.lock().unwrap().clone() }
}

#[derive(Default)]
pub struct TestFs { store: Mutex<HashMap<String, Vec<u8>>> }
...
impl Fs for TestFs {
    ...
    fn write(&self, path: &str, data: &[u8]) -> Result<()> {
        self.store.lock().unwrap().insert(path.into(), data.to_vec());
        Ok(())
    }
    fn list(&self, prefix: &str) -> Result<Vec<String>> {
        Ok(self.store.lock().unwrap().keys()
