# Operating jkr-server

Quick reference for running jkr in production: env flags, the two
...
Set on the `jkr-server` container (via `jkr-server.env` in `docker-compose.yml`).

| Var | What it enables | Default |
|---|---|---|
| `JKR_ANTHROPIC_UPSTREAM` | `/v1/messages` proxy → this URL | unset (503) |
| `JKR_OPENAI_UPSTREAM` | `/v1/chat/completions` proxy → this URL | unset (503) |
| `JKR_LOGTO_{ENDPOINT,APP_ID,APP_SECRET,REDIRECT_URI}` | Logto SSO | unset (sign-in unavailable) |
| `JKR_UPSTREAM_MAX_CONCURRENT` | in-process upstream concurrency cap | `64` |
| `JKR_CAPTURE_BODIES` | stash scrubbed request + response bodies in a ring | `false` |
| `JKR_SANDBOX_EXEC` | expose `POST /api/v1/sandbox/exec` | `false` |
| `JKR_RECEIPT_SIGNING_KEY_PATH` | where the secp256k1 signing key lives on disk | `/var/lib/jkr/receipt-signing-key` |
...
There are two callers of the `jkr-sandbox` crate. Same code, different surfaces, different telemetry. They are easy to confuse — they have the same security primitive (Landlock on Linux, sandbox-exec on macOS) but talk through different channels.

### CLI agent-side (`jkr <cmd>` on a host)

- Runs entirely on the host where the CLI executes.
- Wraps any command (`jkr cargo test`, `jkr npm install`, `jkr bash`) in a per-session jail.
- **No network calls back to jkr-server.** Lives and dies on the host.
...
- Endpoint on jkr-server itself, auth-gated to Logto sessions.
...
Because the CLI doesn't yet POST sandbox events back to jkr-server. There is no telemetry ingest path. Adding one is a separate slice (new server route + CLI-side reporter); until then, agent activity is local-only.
...
Newline-separated, `v1\n` prefix, no trailing newline. Field order is locked by the `signer_canonical_message_is_stable` test in `crates/jkr-server/src/main.rs`.
...
The signing keypair lives on disk at `JKR_RECEIPT_SIGNING_KEY_PATH` (default `/var/lib/jkr/receipt-signing-key`). On first start, jkr-server mints + persists it with `0600` perms. On subsequent starts, the same key loads → signatures stay verifiable across restarts.

**Caveat:** if the path's parent isn't writable (default Docker rootfs with no volume mount), jkr-server falls back to an **ephemeral in-memory key** and logs a warning at startup. Mount a volume at `/var/lib/jkr` for persistence in production.
...
| **Sandbox** | `/api/v1/sandbox/stats` (10s poll) | **Server-side HTTP endpoint only.** Off-default; flip `JKR_SANDBOX_EXEC=true` to enable. |
| **Captured calls** | `/api/v1/llm/captured` (5s poll) | Scrubbed request + response bodies. Off-default; flip `JKR_CAPTURE_BODIES=true` to enable. |
...
- **"My client setup but the dashboard's empty."** Confirm with `curl tkr.prysm.sh/api/v1/filter/stats` — if `total: 0` and `injections_total: 0`, the request never reached jkr-server (DNS / wrong `BASE_URL` / IDE process cached the old config — restart the IDE). If non-zero, jkr saw the request and you should also see entries on `/api/v1/llm/recent`.
- **"Why am I getting 429s?"** Two possible sources: upstream (Anthropic / OpenAI rate-limiting you — the `status` column on Token usage shows their 429) or jkr (`upstream_throttled > 0` in filter stats). If jkr, bump `JKR_UPSTREAM_MAX_CONCURRENT`.
...
- Routes: `crates/jkr-server/src/main.rs` — search for `Method::POST,` to see every endpoint.
...
- Migration schema: `crates/jkr-server/migrations/*.sql`.
